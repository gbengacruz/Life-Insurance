--------------------------------------------------------
--  DDL for Trigger TR_20_CUS_QUST
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "TR_20_CUS_QUST" 
   BEFORE INSERT OR UPDATE ON t_20_cus_qust
   FOR EACH ROW 
 BEGIN 
   IF :new.r_k IS NULL THEN 
    -- SELECT to_number(sys_guid(),'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX') INTO :new.r_k FROM dual; 
    SELECT s_20_cus_qust.NEXTVAL INTO :new.r_k FROM dual; 
   END IF; 
   IF inserting THEN 
...
/
ALTER TRIGGER "TR_20_CUS_QUST" ENABLE;
