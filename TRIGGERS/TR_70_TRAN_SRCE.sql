--------------------------------------------------------
--  DDL for Trigger TR_70_TRAN_SRCE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "TR_70_TRAN_SRCE" 
   BEFORE INSERT OR UPDATE ON t_70_tran_srce
   FOR EACH ROW 
 BEGIN 
   IF :new.r_k IS NULL THEN 
    -- SELECT to_number(sys_guid(),'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX') INTO :new.r_k FROM dual; 
    SELECT s_70_tran_srce.NEXTVAL INTO :new.r_k FROM dual; 
   END IF; 
   IF inserting THEN 
...
/
ALTER TRIGGER "TR_70_TRAN_SRCE" ENABLE;
