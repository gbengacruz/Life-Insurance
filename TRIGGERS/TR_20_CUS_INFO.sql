--------------------------------------------------------
--  DDL for Trigger TR_20_CUS_INFO
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "TR_20_CUS_INFO" 
   BEFORE INSERT OR UPDATE ON t_20_cus_info
   FOR EACH ROW 
 BEGIN 
   IF :new.r_k IS NULL THEN 
    -- SELECT to_number(sys_guid(),'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX') INTO :new.r_k FROM dual; 
    SELECT s_20_cus_info.NEXTVAL INTO :new.r_k FROM dual; 
   END IF; 
   IF inserting THEN 
...
/
ALTER TRIGGER "TR_20_CUS_INFO" ENABLE;
